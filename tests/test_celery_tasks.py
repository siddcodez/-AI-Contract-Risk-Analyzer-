...
class TestCeleryTasks:
    ...
    @patch("app.workers.tasks.analysis_service.analyze_contract", new_callable=AsyncMock)
    @patch("app.workers.tasks.create_async_engine")
    @patch("app.workers.tasks.async_sessionmaker")
    def test_analyze_contract_task_success(
        self,
        mock_sessionmaker: MagicMock,
        mock_create_engine: MagicMock,
        mock_analyze_contract: AsyncMock,
    ) -> None:
        ...
        res = analyze_contract_job(analysis_job_id)

        assert res["status"] == "completed"
        assert res["analysis_job_id"] == analysis_job_id
        mock_analyze_contract.assert_called_once_with(mock_session, uuid.UUID(analysis_job_id))

    @patch("app.workers.tasks.analysis_service.analyze_contract", new_callable=AsyncMock)
    @patch("app.workers.tasks.create_async_engine")
    @patch("app.workers.tasks.async_sessionmaker")
    def test_analyze_contract_task_permanent_validation_error_no_retry(
        self,
        mock_sessionmaker: MagicMock,
        mock_create_engine: MagicMock,
        mock_analyze_contract: AsyncMock,
        ...
        mock_analyze_contract.side_effect = ValidationError("Invalid contract content")

        res = analyze_contract_job(analysis_job_id)

        assert res["status"] == "failed"
        assert "Invalid contract content" in res["error"]

    @patch("app.workers.tasks.analysis_service.analyze_contract", new_callable=AsyncMock)
    @patch("app.workers.tasks.create_async_engine")
    @patch("app.workers.tasks.async_sessionmaker")
    def test_analyze_contract_task_transient_error_retry(
        self,
        mock_sessionmaker: MagicMock,
        mock_create_engine: MagicMock,
        mock_analyze_contract: AsyncMock,
        ...
        mock_analyze_contract.side_effect = StorageError("Storage read failed")

        task_self = MagicMock()
        task_self.request.retries = 0
        task_self.retry.side_effect = Exception("Retry Triggered")

        with patch.object(analyze_contract_job, "retry", task_self.retry):
            try:
                analyze_contract_job(analysis_job_id)
            except Exception as exc:
                assert str(exc) == "Retry Triggered"
